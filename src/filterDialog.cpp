...
wxSizer* FilterDialog::CreateTextBoxes()
{
	...
	mCutoffFrequencyBox = new wxTextCtrl(this, InputTextID, wxString::Format("%0.*f",
		GetPrecision(mParameters.cutoffFrequency, mStringPrecision), mParameters.cutoffFrequency));
	...
	mDampingRatioBox = new wxTextCtrl(this, InputTextID, wxString::Format("%0.*f",
		GetPrecision(mParameters.dampingRatio, mStringPrecision), mParameters.dampingRatio));
	...
	mWidthBox = new wxTextCtrl(this, InputTextID, wxString::Format("%0.*f",
		GetPrecision(mParameters.width, mStringPrecision), mParameters.width));
...
bool FilterDialog::TransferDataFromWindow()
{
	...
	if (!IsZero(steadyStateGain - 1.0) && !IsZero(steadyStateGain))
	{
...
unsigned int FilterDialog::DetermineStringPrecision() const
{
	...
	cutoffSigFig = CountSignificantDigits(mCutoffFrequencyBox->GetValue());

	if (mDampingRatioBox->IsEnabled())
		dampingSigFig = CountSignificantDigits(mDampingRatioBox->GetValue());

	if (mWidthBox->IsEnabled())
		widthSigFig = CountSignificantDigits(mWidthBox->GetValue());
	...
	const double &cutoff) const
{
	...
	unsigned int i;
	for (i = 0; i < order; ++i)
		poles.push_back(std::complex<double>(cutoff, 0.0)
			* std::pow(std::complex<double>(exp(1.0), 0.0),
			std::complex<double>(0.0, (2.0 * (i + 1.0) + order - 1.0) * M_PI / (2.0 * order))));
	...
	const double &cutoff, const double &dampingRatio) const
{
	...
	if (order > 1)
		s = wxString::Format("s^2+%0.*f*s+%0.*f",
			GetPrecision(2.0 * cutoff * dampingRatio, mStringPrecision),
			2.0 * cutoff * dampingRatio,
			GetPrecision(cutoff * cutoff, mStringPrecision), cutoff * cutoff);
	...
	if (order % 2 == 1)
	{
		if (!s.IsEmpty())
			s.Append(_T("*("));
		s.Append(wxString::Format("s+%0.*f", GetPrecision(cutoff, mStringPrecision), cutoff));
		if (s[0] == '(')
			s.Append(_T(")"));
	}
...
//
// Input Arguments:
//		roots	= const std::vector<std::complex<double>>&
//
// Output Arguments:
//		None
//
...
wxString FilterDialog::GenerateExpressionFromComplexRoots(
	const std::vector<std::complex<double>> &roots) const
{
	...
	for (i = 0; i < terms.size(); ++i)
	{
		...
		{
			if (IsZero(terms[i].real() - 1.0))
				coefficient.Clear();
			else
			{
				coefficient.Printf("+%0.*f", GetPrecision(terms[i].real(), mStringPrecision), terms[i].real());
				if (i != terms.size() - 1)
	...
	const double &cutoff, const unsigned int &order) const
{
	...
	numerator = wxString::Format("%0.*f",
		GetPrecision(cutoff, mStringPrecision), pow(cutoff, order));
...
void FilterDialog::GetNotchTF(wxString &numerator, wxString &denominator) const
{
	...
	numerator.Printf("s^2+%0.*f", GetPrecision(cutoff * cutoff, mStringPrecision), cutoff * cutoff);
...
unsigned int FilterDialog::CountSignificantDigits(const wxString& valueString)
{
	double value;
...
bool FilterDialog::IsZero(const double &n, const double &eps)
{
	if (fabs(n) < eps)
		return true;
	else
