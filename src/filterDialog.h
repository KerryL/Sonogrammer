...
struct FilterParameters
{
	...
	enum class Type
	{
		...
		Notch,
		Custom
	} type = FilterParameters::Type::HighPass;///< Type of this filter.
...
private:
	...
	wxString GenerateExpressionFromComplexRoots(const std::vector<std::complex<double>> &roots) const;
	...
	static unsigned int CountSignificantDigits(const wxString& valueString);
	static bool IsZero(const double& v, const double &eps = 1.0e-12);
